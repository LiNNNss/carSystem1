...
public interface CarDao {
    ...
    @Update("update carMessage set carQuantity=#{carQuantity} where id = #{id}")
    int  buyCar(Car car);

    @Select("select * from carMessage where carName like #{name} limit #{beginNum},#{endNum}")
     List<Car> findByCarType(@Param("name") String name , @Param("beginNum") int beginNum,@Param("endNum") int endNum);
